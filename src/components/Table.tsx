import { createMemo, createSignal, For, Show, createEffect, onCleanup, JSX } from 'solid-js'
...
const TableCardSkeleton = (props: { columns: TableColumn<any>[]; rows: number }) => {
  return (
    <For each={Array(props.rows)}>
      {() => (
...
const DefaultCardEmptyState = (props: { columns: TableColumn<any>[]; title?: string; detail?: string }) => {
  return (
    <div class="flex flex-col items-center justify-center h-full text-gray-400">
      <TiCancel class="w-20 h-20 mb-2" />
      <p class="text-lg font-medium">{props.title || 'No data available'}</p>
...
const CardView = <T,>(props: TableProps<T> & { pageSize: number }) => {
  return (
    <div class="min-w-full h-full max-h-[60dvh] overflow-y-auto">
      <div class="relative isolate">
        <div class="grid grid-cols-1 gap-4">
          <Show when={!props.loading} fallback={<TableCardSkeleton columns={props.columns} rows={props.pageSize} />}>
            <Show
            ...
            >
              <For each={props.data}>
                {(row) => (
                  <div
                    class="p-4 bg-void/70 rounded-lg border border-primary/20 shadow-[0_0_25px_rgba(226,254,116,0.1)]"
...
const TableView = <T,>(props: TableProps<T> & { pageSize: number }) => {
  return (
    <div
      ...
      <div class="relative isolate">
        <table class={`w-full ${props.minWidth || 'min-w-fit'}`}>
          ...
          <tbody class="bg-void/70 divide-y divide-primary/10">
            <Show when={!props.loading} fallback={<TableSkeleton columns={props.columns} rows={props.pageSize || 5} />}>
              <Show
                when={props.data.length > 0 && !props.loading}
                fallback={
                  props.emptyFallback ? (
...
const Table = <T,>(props: TableProps<T>) => {
  ...
  const isMobile = createMemo(() => window.innerWidth < 900)
